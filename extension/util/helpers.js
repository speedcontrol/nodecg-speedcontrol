...
var Helpers = /** @class */ (function () {
     ...
     */
    Helpers.formPlayerNamesStr = function (runData) {
        return runData.teams.map(function (team) { return (team.players.map(function (player) { return player.name; }).join(', ')); }).join(' vs. ') || 'No Player(s)';
    };
    /**
     * Takes a run data object and returns an array of all associated Twitch usernames.
     * @param runData Run Data object.
     */
    Helpers.getTwitchChannels = function (runData) {
        var _a;
     ...
     */
    Helpers.padTimeNumber = function (num) {
     ...
     */
    Helpers.timeStrToMS = function (time) {
        var ts = time.split(':');
        if (ts.length === 2) {
     ...
     */
    Helpers.msToTimeStr = function (ms) {
    ...
    Helpers.nullToUndefined = function (varToCheck) {
     ...
     */
    Helpers.nullToNegOne = function (varToCheck) {
     ...
     */
    Helpers.processAck = function (err, ack, data) {
        if (ack && !ack.handled) {
            ack(err, data);
        }
    };
     ...
     */
    Helpers.cgListenForHelper = function (func, ack) {
        func
            .then(function () { Helpers.processAck(null, ack); })
            .catch(function (err) { Helpers.processAck(err, ack); });
    };
    /**
     ...
     */
    Helpers.randomInt = function (low, high) {
